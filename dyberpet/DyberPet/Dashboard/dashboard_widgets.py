...
class itemTabWidget(QWidget):
    ...
    def _sortItems(self):
        """整理背包：按物品级别排序，有物品的格子在前，空格子在后"""
        # 收集所有有物品的格子
        items = []
        for idx, widget in self.cells_dict.items():
            if widget.item_name != 'None':
                item_config = self.items_data.item_dict[widget.item_name]
                fv_lock = item_config.get('fv_lock', 0)
                items.append((widget.item_name, widget.item_num, idx, fv_lock))
        ...
        # 先放有物品的格子
        for new_idx, (item_name, item_count, _, _) in enumerate(items):
            widget = PetItemWidget(new_idx, self.items_data.item_dict[item_name], item_count)
        ...
        # 再放空格子
        n_items = len(items)
        n_empty = max(self.minItemWidget - n_items, 6 - n_items % 6 if n_items % 6 else 0)
        for i in range(n_empty):
            idx = n_items + i
            widget = PetItemWidget(idx)
            widget.Ii_selected.connect(self.change_selected)
            widget.Ii_removed.connect(self.item_removed)
            widget.Ii_pressed.connect(self.item_pressed)
