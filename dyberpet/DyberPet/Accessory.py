...
class DPAccessory(QWidget):
    ...
    def setup_accessory(self, acc_act, pos_x, pos_y, cat_bottom_y=0):
...
class QItemDrop(QWidget):
    ...
    def __init__(self, acc_index,
        ...
        self.acc_act = acc_act
        #self.move(pos_x, pos_y)
        self.size_wh = int(64) # * settings.size_factor)
        ...
        self.setAttribute(Qt.WA_TranslucentBackground, True)
        self.repaint()
        self.resize(self.size_wh, self.size_wh)

        self.move(pos_x - self.size_wh // 2, pos_y)
        ...
        # 使用自动计算的偏移量（兼容多显示器：加上 topLeft.y）
