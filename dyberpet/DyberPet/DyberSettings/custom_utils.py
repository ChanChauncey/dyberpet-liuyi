...
from PySide6.QtWidgets import (QApplication, QWidget, QLabel, QPushButton, QHBoxLayout,
                               QVBoxLayout, QGridLayout, QProgressBar, QFrame,
                               QStyleOptionViewItem, QButtonGroup, QSizePolicy)
...
#===========================================================
#    Customized shortcut list card
#===========================================================

class Dyber_ShortcutCard(SettingCard):
    """ Read-only card listing keyboard shortcuts that operate pet actions """

    def __init__(self, icon, title, shortcut_list, content=None, parent=None):
        """
        Parameters
        ----------
        icon: str | QIcon | FluentIconBase
        ...
        title: str
            the title of card
        shortcut_list: List[Tuple[str, str]]
        ...
        """
        super().__init__(icon, title, content, parent)
        # 隐藏默认内容标签与卡片标题：把横向空间全部让给快捷键列表，
        # 避免标题与列表挤在同一行导致详情被裁。
        self.contentLabel.hide()
        self.titleLabel.hide()

        self.listWidget = QWidget(self)
        self.listLayout = QGridLayout(self.listWidget)
        self.listLayout.setContentsMargins(0, 4, 0, 4)
        self.listLayout.setHorizontalSpacing(10)
        self.listLayout.setVerticalSpacing(8)
        # 第 0 列放按键徽标（固定宽度），第 1 列放说明文字（抢占剩余宽度）
        self.listLayout.setColumnStretch(0, 0)
        self.listLayout.setColumnStretch(1, 1)

        for i, (key, desc) in enumerate(shortcut_list):
            kbadge = QLabel(key)
            kbadge.setAlignment(Qt.AlignCenter)
            kbadge.setMinimumWidth(70)
            kbadge.setMaximumWidth(90)
            kbadge.setStyleSheet(
                "QLabel{background:rgba(128,128,128,0.18);"
                "border:1px solid rgba(128,128,128,0.35);"
                "border-radius:6px;padding:3px 6px;font-weight:600;}"
            )

            dlabel = BodyLabel(desc)
            ...
            self.listLayout.addWidget(kbadge, i, 0, Qt.AlignTop)
            self.listLayout.addWidget(dlabel, i, 1, Qt.AlignTop)

        # 把列表尽量靠左放（去掉前置间距），同时给 listWidget 极大拉伸系数，
        # 让它独占图标右侧的全部剩余宽度，避免详情被挤到右侧裁掉。
        self.hBoxLayout.addWidget(self.listWidget, 100)
        self.hBoxLayout.addSpacing(16)

        # 基类 SettingCard 写死了固定高度（content 存在时 70px），
        # 而本卡片是 9 行快捷键列表，会被裁掉只显示前一两行。
        # 这里按实际内容把卡片高度撑开，确保所有快捷键都可见。
        self.listWidget.adjustSize()
        content_h = self.listWidget.sizeHint().height()
        if content_h > 0:
            # 正常情况：直接用布局算出的真实高度，紧凑无多余空白
            self.setFixedHeight(content_h + 16)
        else:
            # 兜底：极少数情况下 sizeHint 尚未算好（返回 0）时，
            # 按行数估算（每行约 48px，含徽标与换行余量），防止仍被裁掉
            self.setFixedHeight(len(shortcut_list) * 48 + 16)
