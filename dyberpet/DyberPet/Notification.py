...
class DPNote(QWidget):
    ...
    def setup_notification(self, note_type, message=''):
        ...
        if message != '' and settings.toaster_on:
            ...
            if mergeable_type in self.type_dict.keys():
                ...
                # 拉屎通知用棕色背景
                bg_color = 'rgb(220, 180, 130)' if note_type == 'poop' else None
    ...
    def setup_bubbleText(self, bubble_dict, pos_x, pos_y):
        ...
        self.bubble_dict[note_index] = BubbleText(note_index,
                                ...
                                timeout=timeout,
                                countdown = countdown)
        print(f"[气泡调试] 创建气泡: timeout={timeout}ms, countdown={countdown}, leftover={timeout//1000}s")
...
class DyberToaster(QFrame):
    ...
    def __init__(self, note_index,
        ...
        self.bg_color = bg_color or 'rgb(255, 255, 255)'
...
class BubbleText(QFrame):
    ...
    def __init__(self, note_index,
        ...
        if self.countdown:
            ...
            # 鼠标位置检测定时器（用于鼠标靠近隐藏/离开恢复）
            self._mouse_check_timer = QTimer(timeout=self._check_mouse_position)
            self._mouse_hover_ani = QPropertyAnimation(self, b'windowOpacity')
            self._mouse_hover_ani.setDuration(200)
            self._mouse_hover_visible = True
    ...
    def _check_mouse_position(self):
        ...
        mouse_in = self.geometry().contains(cursor_pos)

        if mouse_in and self._mouse_hover_visible:
            # 鼠标进入 → 淡出隐藏
            self._mouse_hover_visible = False
            self._mouse_hover_ani.stop()
            self._mouse_hover_ani.setStartValue(self.windowOpacity())
            self._mouse_hover_ani.setEndValue(0.0)
            self._mouse_hover_ani.start()
            self.setAttribute(Qt.WA_TransparentForMouseEvents, True)
        elif not mouse_in and not self._mouse_hover_visible:
            # 鼠标离开 → 淡入显示
            self._mouse_hover_visible = True
            self._mouse_hover_ani.stop()
            self._mouse_hover_ani.setStartValue(self.windowOpacity())
            self._mouse_hover_ani.setEndValue(1.0)
            self._mouse_hover_ani.start()
