...
class PetConfig:
    ...
    def __init__(self):
        ...
        self.restore_btn_offset = [0, 0]
        self.restore_btn_offset_mirror = [0, 0]
    ...
    def init_config(cls, pet_name: str, pic_dict: dict):
        ...
        with open(path, 'r', encoding='UTF-8') as f:
            ...
            act_dict = {}
            act_dict = {k: LazyAct(v, o.scale, pet_name, 'role', k) for k, v in act_conf.items()}
            o.act_dict = act_dict
            # 载入默认动作
            ...
            o.drag_start = act_dict[conf_params.get('drag_start', conf_params['drag'])]
            o.drag = act_dict[conf_params['drag']]
            o.fall = act_dict[conf_params['fall']]
            o.fall_loop = act_dict[conf_params.get('fall_loop', 'fall')]
            ...
            for act_array in conf_params['random_act']:
                ...
                raw = act_array.get('act_type', 0)
                # 兼容新格式（单数字）和旧格式（数组）
                act_type.append([2, raw] if isinstance(raw, (int, float)) else raw)
            ...
            o.restore_btn_offset = conf_params.get('restore_btn_offset', [0, 0])
            o.restore_btn_offset_mirror = conf_params.get('restore_btn_offset_mirror', [0, 0])
    ...
    def init_subpet(cls, pet_name: str, pic_dict: dict):
        ...
        with open(path, 'r', encoding='UTF-8') as f:
            ...
            o.left = act_dict[conf_params.get('left', 'default')]
            o.right = act_dict[conf_params.get('right', 'default')]
            o.drag_start = act_dict[conf_params.get('drag_start', conf_params.get('drag', 'default'))]
...
class Act:
    def __init__(self, images=(), act_name=None, act_num=1, need_move=False, direction=None, frame_move=10, frame_refresh=0.04, anchor=[0,0], move_phases=None):
        """
        动作
        :param images: 动作图像
        ...
        self.anchor = anchor
        self.move_phases = move_phases
        self.frame_start = 0
        self.frame_end = len(images) - 1 if images else 0
    ...
    def init_act(cls, conf_param, pic_dict, scale, pet_name, resFolder='role', act_name=None):
        ...
        # 支持帧范围裁剪：frame_start/frame_end 是原始素材中的帧索引
        frame_start = conf_param.get('frame_start', None)
        frame_end = conf_param.get('frame_end', None)
        if frame_start is not None and frame_end is not None:
            img = img[frame_start:frame_end + 1]

        move_phases = conf_param.get('move_phases', None)
        act = Act(img, act_name, act_num, need_move, direction, frame_move, frame_refresh, anchor, move_phases)
    ...
    def customized_copy(self, start_idx, end_idx, num_rep):
        ...
        return Act(imgs, self.act_name, num_rep, self.need_move, self.direction, self.frame_move, self.frame_refresh, self.anchor, self.move_phases)


class LazyAct:
    """懒加载动作：只保存配置，首次访问 images 时才加载图片"""
    _cache = {}  # 类级别缓存，所有实例共享

    def __init__(self, conf_param, scale, pet_name, resFolder='role', act_name=None):
        self._conf_param = conf_param
        self._scale = scale
        self._pet_name = pet_name
        self._resFolder = resFolder
        self.act_name = act_name
        self.act_num = conf_param.get('act_num', 1)
        self.need_move = conf_param.get('need_move', False)
        self.direction = conf_param.get('direction', None)
        self.frame_move = conf_param.get('frame_move', 10) * scale
        self.frame_refresh = conf_param.get('frame_refresh', 0.5)
        self.anchor = conf_param.get('anchor', [0,0])
        self.move_phases = conf_param.get('move_phases', None)
        self._frame_start = conf_param.get('frame_start', None)
        self._frame_end = conf_param.get('frame_end', None)
        self._images = None
        self._loaded = False
    ...
    def _load_images(self):
        """首次访问时加载图片"""
        cache_key = (self._pet_name, self._resFolder, self._conf_param.get('images'))
        if cache_key in LazyAct._cache:
            img = LazyAct._cache[cache_key]
        else:
            images = self._conf_param['images']
            ...
            for i in matching_idx:
                pixmap = QPixmap()
                pixmap.load(os.path.join(img_dir, f'{images}_{i}.png'))
                if not pixmap.isNull():
                    img.append(pixmap)

            if self._scale != 1:
                img = [i.scaled(int(i.width() * self._scale),
                                int(i.height() * self._scale),
                                aspectMode=Qt.KeepAspectRatio,
                                mode=Qt.SmoothTransformation) for i in img]

            LazyAct._cache[cache_key] = img

        # 帧范围裁剪
        if self._frame_start is not None and self._frame_end is not None:
            img = img[self._frame_start:self._frame_end + 1]

        self._images = img
        self._loaded = True
        if self._images:
            self.frame_end = len(self._images) - 1
        else:
            self.frame_end = 0
    ...
    def frame_end(self):
        if not self._loaded:
            self._load_images()
        return self._frame_end if self._frame_end is not None else (len(self._images) - 1 if self._images else 0)
...
class ActData:
    ...
    def _get_act_config(self, actset, act_type, fv_lvl):
        raw = actset.get('act_type', 0)
        # 兼容新格式（单数字）和旧格式（数组）
        if isinstance(raw, list):
            status_type = raw
        else:
            status_type = [2, raw]  # 单数字 = fv_level 门槛，hp_tier 用默认值 2
        ...
        return {
                ...
                "entertainment": actset.get('entertainment', False)
                }
...
class PetData:
    ...
    def _check_items(self, data_params):
        ...
        for item, value in data_params['items'].copy().items():
            ...
            # Fix None index (data corruption or import issue)
            if idx is None:
                idx = new_idx
