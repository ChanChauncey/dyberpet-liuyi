...
def process_video(video_name: str):
    ...
    if rgba_dir.exists():
        existing_frames = list(rgba_dir.glob("*_rgba.png"))
        if len(existing_frames) > 50:  # 假设超过50帧就是已完成
            print(f"\n跳过 {video_name} - 已有 {len(existing_frames)} 帧")
            return
    ...
    # 步骤1：提取帧（如果不存在）
    if not frame_dir.exists() or len(list(frame_dir.glob("frame_*.png"))) == 0:
        print(f"\n步骤1: 提取帧...")
