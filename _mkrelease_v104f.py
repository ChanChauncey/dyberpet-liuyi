...
EXE = r"C:\DyberPet\dist_logo_inst\LiuYi_Setup_v1.0.4.exe"
SRC_ZIP = r"C:\DyberPet\DyberPet_source.zip"
TAG = "v1.0.4"
BODY = (
    ...
    "- On confirm: auto-download with progress (domestic GitHub mirrors first, official fallback),\n"
    "  silent install, auto-restart, data kept\n"
    "- Differential update: auto-updates download only the small source patch\n"
    "  (DyberPet_source.zip, ~240KB) instead of the full ~247MB installer\n"
    "- Check-update result shown at top of window; current version shown in About\n\n"
    "Download LiuYi_Setup_v1.0.4.exe to install. Existing users get the small patch automatically."
)
...
# 4) 上传完整安装包（约 247MB，可能耗时）
print("=== uploading LiuYi_Setup_v1.0.4.exe ===", file=sys.stderr)
asset_url = up + "?name=LiuYi_Setup_v1.0.4.exe"
...
if os.path.isfile(SRC_ZIP):
    ...
    rc3, out3, err3 = curl("POST", asset_url2,
        {"Authorization": f"Bearer {tok}", "Content-Type": "application/octet-stream"},
        data=SRC_ZIP, binary=True, timeout=300)
    print("upload src rc=", rc3, "err=", err3[:300], file=sys.stderr)
    print("upload src resp=", out3[:300], file=sys.stderr)
else:
    print("WARNING: DyberPet_source.zip not found, skip patch upload", file=sys.stderr)
